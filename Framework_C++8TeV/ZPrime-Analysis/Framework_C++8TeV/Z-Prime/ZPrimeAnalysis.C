        		...
        		if(goodjet_n >= 4)
        		  {
                    ...
                    // Definitions of Jets
                    TLorentzVector JetOne = TLorentzVector();
                    TLorentzVector JetTwo = TLorentzVector();
                    TLorentzVector JetThree = TLorentzVector();
                    TLorentzVector JetFour = TLorentzVector();

                    TLorentzVector MomentumSum = TLorentzVector();
                    TLorentzVector FirstTwoJets = TLorentzVector();
                    TLorentzVector FirstThreeJets = TLorentzVector();

                    //Filling Jets wityh highest pT
                    JetOne.SetPtEtaPhiE(jet_pt[0], jet_eta[0], jet_phi[0],jet_E[0]);
                    JetTwo.SetPtEtaPhiE(jet_pt[1], jet_eta[1], jet_phi[1],jet_E[1]);
                    JetThree.SetPtEtaPhiE(jet_pt[2], jet_eta[2], jet_phi[2],jet_E[2]);
                    JetFour.SetPtEtaPhiE(jet_pt[3], jet_eta[3], jet_phi[3],jet_E[3]);

                    MomentumSum = JetOne + JetTwo + JetThree + JetFour + Lepton_1 + MeT;
                    FirstTwoJets = JetOne + JetTwo;
                    FirstThreeJets = JetOne + JetTwo + JetThree;

//Calculating masses
                    float ZPrimeMass = MomentumSum.Mag();
                    float FirstTwoJetsMass = FirstTwoJets.Mag();
                    float FirstThreeJetsMass = FirstThreeJets.Mag();

            				double names_of_global_variable[]={InvMass1_inGeV, missingEt, vxp_z, (double)pvxp_n, Lepton1_MeT_MT/1000.,ZPrimeMass/1000.,FirstTwoJetsMass/1000.,FirstThreeJetsMass/1000.};
            				...
            				TString histonames_of_global_variable[]={"hist_vismass","hist_etmiss","hist_vxp_z","hist_pvxp_n", "hist_mt", "hist_hist_ZPrimeMass", "hist_FirstTwoJetsMass", "hist_FirstThreeJetsMass"};
